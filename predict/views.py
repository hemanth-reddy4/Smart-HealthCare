...
def index(request):
    ...
    try:
        ...
        response.raise_for_status()  # Ensures a successful response
        ...
        if len(quote_list) > 17:
            ...
            # Provide default values or handle error as needed
            quote = "Medical wisdom shared here."
            author = "Unknown"
    
    except requests.RequestException as e:
        # Handle request errors (e.g., network issues, invalid response)
        print(f"Error fetching quote: {e}")
        quote = "Could not retrieve a medical quote at this time."
        author = "N/A"
...
def prediction(request):
    ...
    # Training data
    path_train = "C:\\Users\\LENOVO\\Downloads\\Health Care\\smart-healthcare-system\\predict\\templates\\predict\\Training.csv"


    data_training = pd.read_csv(path_train)
    path_test = "C:\\Users\\LENOVO\\Downloads\\Health Care\\smart-healthcare-system\\predict\\templates\\predict\\Testing.csv"
    ...
    #output_decision = clf3.predict(list_symptoms)
    #output_random = clf4.predict(list_symptoms)
    #output_naive = gnb.predict(list_symptoms)
    # Print the predictions from each model
    #print("Decision Tree Prediction:", output_decision[0])
    #print("Random Forest Prediction:", output_random[0])
    #print("Naive Bayes Prediction:", output_naive[0])

# You can combine the predictions or use a voting system to determine the final prediction
# For example, you could use a majority vote:
    #if output_decision[0] == output_random[0] == output_naive[0]:
        #predicted_disease = output_decision[0]
    #else:
    # Handle cases where predictions disagree, e.g., prioritize one model or use a probabilistic approach
       # predicted_disease = output_naive[0]  # You can choose a different model or implement a voting system
